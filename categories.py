...
# Заметка: «Снятие/Внесение наличных» и «Пополнение с карты» определяются
# не по описанию, а по типу операции в default_category — это надёжнее,
# потому что «банкомат» в описании может означать и снятие, и пополнение.
...
def default_category(tx_type: str, amount: float) -> str:
    """Дефолт, когда правила по описанию не сработали.
    Опирается на тип операции и знак — самый надёжный признак."""
    t = (tx_type or "").lower()
    if "перевод" in t or "transfer" in t:
        return "Перевод (входящий)" if amount > 0 else "Перевод (исходящий)"
    if "снятие" in t:
        return "Внесение наличных" if amount > 0 else "Снятие наличных"
    if "пополнение" in t or "зачисление" in t:
        return "Пополнение (с карты)" if amount > 0 else "Прочее"
    if amount > 0:
